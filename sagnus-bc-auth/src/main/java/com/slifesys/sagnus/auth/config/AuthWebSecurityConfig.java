...
import com.slifesys.sagnus.auth.infrastructure.security.ErpAccessDeniedHandler;
import com.slifesys.sagnus.auth.infrastructure.security.ErpAuthenticationEntryPoint;
...
public class AuthWebSecurityConfig {
     ...
     *
     * OBS: O mecanismo de autenticação (JWT filter / authentication provider) deve vir do
     * sagnus-platform-security (ou configuração equivalente). Este filtro deve popular o SecurityContext.
     * A autorização fina (ROLE_AUTH_ADMIN etc.) continua via @PreAuthorize nos métodos.
     */
    @Bean
