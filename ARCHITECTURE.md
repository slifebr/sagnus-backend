...
├─ sagnus-bc-corp-api              # Contratos CORP (ports + DTOs estáveis)
│
...
**Contratos:** `sagnus-bc-corp-api` expõe portas e DTOs mínimos e estáveis, por exemplo:
- `CorpPessoaQueryPort`
...
- AUTH usa CORP apenas via contrato (`sagnus-bc-corp-api`) para:
  - validar `pessoa_id`
...
- `bc-auth` → pode depender de `bc-corp-api` (contrato), **não** de `bc-corp`
- `bc-nfe` → pode depender de `bc-corp-api` (contrato), **não** de `bc-corp`
...
Se você quiser, o próximo documento natural é:
- `DEVELOPMENT.md` (onboarding: como rodar, perfis, seed, comandos)
- `DECISIONS.md` (ADR: decisões arquiteturais e por quê)
